...
// GET /api/admin/achievement-levels - List achievement levels
...
// POST /api/admin/achievement-levels - Create achievement level
export async function POST(request: NextRequest) {
  try {
    ...
    // Check duplicates
    const existing = await dbAll('SELECT id FROM achievement_levels WHERE name = ?', [name]);
    ...
    const result = await dbRun(
      `INSERT INTO achievement_levels (name, multiplier, description, created_at)
       VALUES (?, ?, ?, datetime('now'))`,
