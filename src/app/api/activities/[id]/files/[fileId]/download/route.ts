...
function resolveActivityPublicFilePath(
  ...
  return { fullPath, relative: relNormalized };
}

// GET /api/activities/:id/files/:fileId/download - Download file
export async function GET(
  ...
  try {
    ...
    const attachment = await dbGet(
      'SELECT id, file_path, file_name, mime_type FROM activity_attachments WHERE id = ? AND activity_id = ?',
    ...
    console.error('Download activity file error:', error);
