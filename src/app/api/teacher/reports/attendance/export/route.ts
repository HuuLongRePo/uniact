...
function normalizeMethodFilter(value: unknown): 'manual' | 'qr' | 'face' | null {
...
function normalizeStatusFilter(value: unknown): 'attended' | 'absent' | 'registered' | null {
  const normalized = String(value || '')
    .trim()
    .toLowerCase();
  if (normalized === 'attended' || normalized === 'absent' || normalized === 'registered') {
    return normalized;
  }
  return null;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const filters =
      requestBody && typeof requestBody === 'object' && requestBody.filters
        ? requestBody.filters
        : {};

    const selectedClassName =
      filters && typeof filters.classId === 'string' ? filters.classId.trim() : '';
    const selectedStatus = normalizeStatusFilter(filters?.status);
    const selectedMethod = normalizeMethodFilter(filters?.method);
    const dateStart =
      filters && typeof filters.dateStart === 'string' ? filters.dateStart.trim() : '';
    const dateEnd = filters && typeof filters.dateEnd === 'string' ? filters.dateEnd.trim() : '';
    ...
    const whereClauses = [
      "u.role = 'student'",
      `u.class_id IN (${inClause})`,
      "date(a.date_time) >= date('now', '-365 day')",
    ];
    const queryParams: Array<string | number> = [...classIds];

    if (selectedClassName) {
      whereClauses.push('c.name = ?');
      queryParams.push(selectedClassName);
    }

    if (selectedStatus) {
      whereClauses.push('p.attendance_status = ?');
    ...
    if (selectedMethod) {
      whereClauses.push(
        `EXISTS (
          SELECT 1
          FROM attendance_records ar_filter
          WHERE ar_filter.activity_id = a.id
            AND ar_filter.student_id = u.id
            AND lower(ar_filter.method) = ?
        )`
      );
      queryParams.push(selectedMethod);
    }

    if (dateStart) {
      whereClauses.push('date(a.date_time) >= date(?)');
