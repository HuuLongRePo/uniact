...
**Response format (quy ước đề xuất):**

Để thuận tiện cho client và kiểm thử, hệ thống thống nhất phản hồi theo một cấu trúc chung (có thể điều chỉnh khi triển khai):

- Thành công: `{ success: true, data, message? }`
- Thất bại: `{ success: false, error, message? }`
...
**1) Duyệt hoạt động**
- `GET /api/activity-approvals` → `{ approvals: [...] }`
- `POST /api/activity-approvals` body `{ approval_id, action: 'approved'|'rejected', note? }` → `{ success: true }`
