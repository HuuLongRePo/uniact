...
## 2026-04-12 - Hoàn thành T-154

### Đã làm

- Canonicalize `src/app/api/admin/activities/route.ts`:
  - chuyển từ `getUserFromRequest` sang `requireApiRole(request, ['admin'])`
  - đổi raw `NextResponse` sang `successResponse` / `errorResponse`
  - validate rõ `teacher_id` filter
  - bỏ derive `pending/rejected` trực tiếp trong SQL, thay bằng `getActivityDisplayStatus(...)` ở tầng response
  - preserve `ApiError` / API-shaped errors thay vì trả raw unauthorized/internal error
- Cập nhật `src/app/admin/activities/page.tsx` để đọc tốt hơn canonical success shape (`data.activities`) trong khi vẫn tương thích top-level hiện tại
- Thêm test mới `test/admin-activities-route.test.ts` để khóa:
  - canonical success shape + pending display status từ `approval_status=requested`
  - preserve forbidden error shape
  - invalid `teacher_id` filter trả validation error canonical

### Kiểm thử

- Chạy `npm test -- --reporter dot test/admin-activities-route.test.ts test/admin-activity-detail-route.test.ts test/admin-activity-participants-route.test.ts test/admin-approval-action-route.test.ts test/admin-approval-history-route.test.ts test/admin-pending-activities-route.test.ts test/teacher-edit-activity-page.test.tsx test/teacher-create-activity-page.test.tsx test/teacher-create-activity-preview.test.tsx test/teacher-edit-activity-preview.test.tsx test/student-activity-detail-page.test.tsx test/register-route-conflict.test.ts test/register-route-mandatory.test.ts test/register-route-cancel-route.test.ts test/student-activities-page.test.tsx test/activities-list-route.test.ts test/my-registrations-route.test.ts test/teacher-approvals-route.test.ts test/teacher-resubmit-route.test.ts test/activity-check-conflicts-route.test.ts`
- Kết quả: `20/20` test files pass, `40/40` tests pass

### Kết quả

- Admin activities surface giờ đã được harden khá đồng bộ ở list/detail/participants/pending
- Display status cho admin UI vẫn đúng, nhưng workflow truth không còn bị nhúng lẫn với SQL legacy
- Đây là một bước release-prep đáng kể vì giảm mạnh drift giữa các route trung tâm của admin surface

## 2026-04-12 - Hoàn thành T-155

### Đã làm

- Canonicalize `src/app/api/teacher/dashboard/route.ts`:
  - chuyển từ `getUserFromSession()` sang `requireApiRole(request, ['teacher'])`
  - preserve `ApiError` / API-shaped errors ở catch thay vì luôn collapse về internal error
- Canonicalize `src/app/api/teacher/dashboard-stats/route.ts`:
  - chuyển sang `requireApiRole(request, ['teacher'])`
  - preserve forbidden/API-shaped errors ở catch cuối
- Cập nhật `test/teacher-dashboard-routes.test.ts` để mock `requireApiRole` thay cho session auth cũ và khóa rõ canonical forbidden shape
- Thêm `test/teacher-dashboard-legacy-auth-route.test.ts` để khóa riêng compatibility dashboard route không còn nuốt forbidden thành 500

### Kiểm thử

- Chạy `npm test -- --reporter dot test/teacher-dashboard-routes.test.ts test/teacher-dashboard-legacy-auth-route.test.ts test/teacher-approvals-route.test.ts test/teacher-resubmit-route.test.ts test/teacher-edit-activity-page.test.tsx test/teacher-create-activity-page.test.tsx test/admin-activities-route.test.ts test/admin-activity-detail-route.test.ts test/admin-activity-participants-route.test.ts test/admin-approval-action-route.test.ts test/admin-approval-history-route.test.ts test/admin-pending-activities-route.test.ts test/student-activity-detail-page.test.tsx test/register-route-conflict.test.ts test/register-route-mandatory.test.ts test/register-route-cancel-route.test.ts test/student-activities-page.test.tsx test/activities-list-route.test.ts test/my-registrations-route.test.ts test/activity-check-conflicts-route.test.ts`
- Kết quả: `20/20` test files pass, `42/42` tests pass

### Kết quả

- Teacher dashboard surface đã bớt lệch khỏi backbone canonical ở phần auth và error handling
- Lỗi guard không còn bị đẩy thành 500 giả, giúp behavior teacher surface nhất quán hơn với admin/student routes đã harden trước đó
- Batch này tiếp tục làm sạch các bề mặt gần release thay vì quét vụn các route ít quan trọng

## 2026-04-12 - Hoàn thành T-156

### Đã làm

- Canonicalize `src/app/api/teacher/students/route.ts`:
  - chuyển từ `getUserFromSession()` sang `requireApiRole(request, ['teacher'])`
  - preserve `ApiError` / API-shaped errors ở catch cuối
  - giữ payload canonical nhưng thêm compatibility aliases cho teacher UI hiện tại (`full_name`, `total_score`, `activity_count`, `attended_count`)
- Cập nhật `src/app/teacher/students/page.tsx`:
  - đọc tốt hơn canonical success shape (`data.students`) trong khi vẫn tương thích top-level hiện tại
  - giảm phụ thuộc cứng vào field cũ bằng fallback sang `name`, `total_points`, `activities_count`
  - làm logic sort/filter chịu được contract chuyển tiếp tốt hơn
- Thêm `test/teacher-students-route.test.ts` để khóa:
  - canonical success shape + compatibility aliases cho UI teacher students
  - preserve forbidden error shape
  - empty state canonical khi teacher chưa được phân công lớp nào

### Kiểm thử

- Chạy `npm test -- --reporter dot test/teacher-students-route.test.ts test/teacher-dashboard-routes.test.ts test/teacher-dashboard-legacy-auth-route.test.ts test/teacher-approvals-route.test.ts test/teacher-resubmit-route.test.ts test/teacher-edit-activity-page.test.tsx test/teacher-create-activity-page.test.tsx test/admin-activities-route.test.ts test/admin-activity-detail-route.test.ts test/admin-activity-participants-route.test.ts test/admin-approval-action-route.test.ts test/admin-approval-history-route.test.ts test/admin-pending-activities-route.test.ts test/student-activity-detail-page.test.tsx test/register-route-conflict.test.ts test/register-route-mandatory.test.ts test/register-route-cancel-route.test.ts test/student-activities-page.test.tsx test/activities-list-route.test.ts test/my-registrations-route.test.ts test/activity-check-conflicts-route.test.ts`
- Kết quả: `21/21` test files pass, `45/45` tests pass

### Kết quả

- Teacher students surface bớt lệch rõ rệt giữa API và page consumer
