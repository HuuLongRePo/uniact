...
describe('StudentHistoryPage', () => {
  ...
  it('renders canonical nested history payload', async () => {
    ...
    expect(screen.getByText('Điểm danh khuôn mặt')).toBeInTheDocument();
  ...
  it('surfaces canonical api errors when history fetch fails', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('student-history-empty-state')).toBeInTheDocument();
    });
  });
});
