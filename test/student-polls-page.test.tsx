...
describe('StudentPollsPage', () => {
    ...
    const fetchMock = vi.fn(async () => ({
      ...
      json: async () => ({
        polls: [
          {
            id: 7,
            title: 'Khao sat hoc ky',
            description: 'Danh gia hoc ky',
  ...
  it('opens poll detail and submits vote with canonical endpoint', async () => {
      ...
      if (url === '/api/polls/7' && !init?.method) {
        return {
          ...
          json: async () => ({
            ...
            options: [{ id: 1, option_text: 'Dong y', vote_count: 0, percentage: '0' }],
            total_votes: 2,
            user_votes: [],
            has_voted: false,
          }),
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'Tham gia poll' }))
